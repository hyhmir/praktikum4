...
# ╔═╡ 18e87c50-39b7-45bc-b513-effcc1f46e7b
data.time = runmean(data.Source, 100);

# ╔═╡ 432e7489-1fdf-4a9f-8604-d52ef2763dc6
data.U1 = runmean(data.CH1, 256);

# ╔═╡ 5be4559a-50a8-43ab-ab9e-e51c077d4b00
data.U2 = runmean(data.CH2, 256);

# ╔═╡ 06da99ed-0674-41ff-a113-d946d814078c


# ╔═╡ 04a85db5-1716-4768-83ff-7994468a7aba
model1(x, p) = p[1].*cos.((p[2]).*x).*sin.((p[3]).*x .+ p[4]).*exp.(p[5].*x) .+ p[6];

# ╔═╡ 65d3c8fa-3b9a-40af-a97a-28baf87f60ca
model2(x, p) = p[1].*sin.((p[2]).*x).*sin.((p[3]).*x .+ p[4]).*exp.(p[5].*x) .+ p[6];
...
# ╔═╡ ce2e8300-e8e0-4a3a-8b8e-0d3721840bfd
p1 = plot(data.time, data.U1, label="meritev U1");
