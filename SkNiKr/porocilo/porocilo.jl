...
# ╔═╡ 8e831460-18ed-41d2-ba69-90bfda69c332
using RollingFunctions, Statistics, LsqFit, CSV, DataFrames, Plots

# ╔═╡ 74b8ee8b-f574-4316-a835-53da32dc506c
using PlutoUI

# ╔═╡ 770a095b-6cf1-4fd7-b03a-45a8bf956fce
using Images
...
# ╔═╡ da2e184b-3f09-499a-bf01-712c99a57655
df_rolling = mapcols(col -> [mean(col[i:min(i+100-1, end)]) for i in 1:length(col)], df_filtered);

# ╔═╡ 14641329-75e6-43ee-824b-19d06dc8b18d
model1(x, p) = p[1].*cos.((p[2] - p[3]).*x).*sin.((0.5*p[2] + 0.5*p[3]).*x .+ p[4]).*exp.(p[5].*x) .+ p[6];

# ╔═╡ fee19be5-d0d8-4ab7-8b15-eafc521ae04a
model1_o(x, p) = p[1].*cos.((p[2] - p[3]).*x).*exp.(p[5].*x) .+ p[6];

# ╔═╡ f35107a7-6cd8-4e3b-b0dd-71b58d9418b3
model2(x, p) = p[1].*sin.((p[2] - p[3]).*x).*sin.((0.5*p[2] + 0.5*p[3]).*x .+ p[4]).*exp.(p[5].*x) .+ p[6];

# ╔═╡ f27dc9da-d457-49a4-99b6-55494e50135c
model2_o(x, p) = p[1].*sin.((p[2] - p[3]).*x).*exp.(p[5].*x) .+ p[6];

# ╔═╡ e06f8fe6-a2a7-487a-b949-7a4e4b912892
model3(x, p) = p[1].*(p[2].*x).*sin.(p[3].*x .+ p[4]).*exp.(p[5].*x) .+ p[6];

# ╔═╡ 105b6b88-1230-47e8-9270-1e1a1610c32f
model3_o(x, p) = p[1].*(p[2].*x).*exp.(p[5].*x) .+ p[6];

# ╔═╡ 3e9ee5be-b523-41d4-bef8-568290762caa
p0 = [0.01, 4e5, 3.5e5, 0, -9576.18, 0.0001];
...
manifest_format = "2.0"
project_hash = "3ef886ad6c5164bac0e646c572c5098cfd48782c"
